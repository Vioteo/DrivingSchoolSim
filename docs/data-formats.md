...
- `DriverCommand` struct: `long sequence`; `float steering, throttle, brake, clutch`; `bool handbrake, ignition, starter`; `int requestedGear`; `AutomaticSelector selector` (P/R/N/D, по умолчанию P; МКПП его игнорирует); `TurnSignal turnSignal` (Off/Left/Right — положение подрулевого переключателя, не выводится из угла руля); `bool hazard`; `HeadlightMode headlights` (Off/Parking = габариты/LowBeam = ближний); `bool highBeam` (дальний поверх ближнего), `flashHighBeam` (моргнуть дальним), `horn`, `seatbelt`, `washer`; `WiperMode wipers` (Off/Interval/Low/High). Steering −1…1, педали 0…1, clutch 1 = выжато (разомкнуто), gear −1 R / 0 N / 1…6. `void Validate()` отклоняет NaN/∞, выход за диапазоны и неопределённые значения enum. Pause по-прежнему не входит в команду.
- `VehicleState` struct: `long tick`, `double simulationSeconds`, `float signedSpeedMps, engineRpm, steeringRadians, clutchTorqueNm`; `int gear`; `EnginePhase engine`; `bool leftIndicator, rightIndicator` (сигнал включён, в т.ч. аварийкой), `indicatorLampOn` (фаза мигания 1,5 Гц), `lowBeam, highBeam, parkingLights, brakeLight, reverseLight, hazard, horn, seatbelt, handbrake`; `TransmissionType transmission`; `AutomaticSelector selector`; `WiperMode wipers`, `float wiperAngle01` (0 = щётки в парковке, 1 = крайнее положение); `float engineTorqueNm, wheelSpeedFrontRadS, wheelSpeedRearRadS`; `float fuelLitres` (топливо в баке, л), `fuelFlowLitresPerHour` (мгновенный расход, л/ч; 0 на принудительном холостом ходу). Нет pose и pedal feedback. `EnginePhase`: Off, Ignition, Cranking, Running, Stalled.
- Перечисления (`Contracts/VehicleControls.cs`): `TransmissionType {Manual, Automatic}`, `AutomaticSelector {P, R, N, D}`, `TurnSignal {Off, Left, Right}`, `HeadlightMode {Off, Parking, LowBeam}`, `WiperMode {Off, Interval, Low, High}`.
- `WeatherConditions` struct (`Contracts/WeatherConditions.cs`): `WeatherPreset preset` (ClearDay, Overcast, Rain, HeavyRain, Fog, Snow, ClearNight, RainNight, FullMoonNight — ясная ночь с большой низкой луной); `float rainIntensity01, snowIntensity01, fogDensity01, timeOfDayHours`; `SurfaceType surface` (DryAsphalt, WetAsphalt, PackedSnow, BlackIce); `bool IsNight`; `static FromPreset(WeatherPreset)`. Значения пресетов — игровая калибровка, не метеоданные.
- Не в Contracts, но сериализуемо (`DS.Simulation`): `VehicleSpec` — калибровка автомобиля; имена полей совпадают с `StreamingAssets/Examples/vehicle.json` (`massKg, wheelbaseM, trackM, wheelRadiusM, centreOfMassM[3], steeringWheelDegrees, gearRatios[], reverseRatio, finalDrive, idleRpm, redlineRpm, engineInertiaKgm2, maxClutchTorqueNm`) плюс новые с умолчаниями (`maxSteerDeg, drivetrainEfficiency, maxBrakeTorqueNm, frontBrakeShare, handbrakeTorqueNm, wheelInertiaKgm2, yawInertiaKgm2, rollingResistance, dragCoefficient, frontalAreaM2, throttleProgression, abs, drive, transmission, fuelTankLitres` = 50, `initialFuelLitres` = 40). `WheelContact` (вход солвера на колесо: `grounded, normalForceN, velocityRightMps, velocityForwardMps, frictionCoefficient`, оси кузова) и `WheelOutput` (силы в осях кузова, скорость/угол вращения, угол поворота, скольжения).
