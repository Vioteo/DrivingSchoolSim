...
Набор `DS_Pedestrian_*` — люди с реалистичными пропорциями (без текстур, однотонные материалы), общим скелетом и зацикленными клипами «на месте». Навигации, ИИ и логики перехода здесь нет (это T17), набор даёт только внешний вид и анимации.

## Состав

| Ассет | Кто | Рост, м | Клипы |
|---|---|---|---|
| `DS_Pedestrian_A` | мужчина, куртка, короткая стрижка | 1.78 | Idle, Walk, Run, LookAround |
| `DS_Pedestrian_B` | женщина, куртка, волосы до плеч | 1.66 | Idle, Walk, Run, LookAround |
| `DS_Pedestrian_C` | мужчина плотнее сложением, с рюкзаком | 1.82 | Idle, Walk, Run, LookAround |
| `DS_Pedestrian_Child_A` | школьник с ранцем (~8 лет) | 1.28 | Idle, Walk, Run, LookAround |
| `DS_Pedestrian_Child_B` | школьница с хвостиками (~8 лет) | 1.26 | Idle, Walk, Run, LookAround |
| `DS_Pedestrian_Police` | инспектор ДПС: форма, жилет со световозвращающими полосами, фуражка, жезл | 1.80 (с фуражкой выше) | Idle, Walk, LookAround, Signal_ArmsSide, Signal_RightArmForward, Signal_ArmUp |
...
- Геометрия — `tools/pedestrian_body.py`. Пропорции задаются долями роста для трёх типов сложения (мужчина, женщина, ребёнок ~8 лет): голова ≈1/7.7 роста у взрослого и ≈1/6 у ребёнка, ширина плеч, длины сегментов рук и ног, стопа. Это типовые антропометрические соотношения для правдоподобия, а не норматив.
- Тело в одежде, шея, голова, кисти, обувь и объём волос — семейства metaball: элементы вдоль скелета сливаются в одну гладкую поверхность. Руки, ноги, шея — отдельные семейства (не срастаются с корпусом и друг с другом), стык скрыт одеждой и воротником. Из того же поля считаются веса скиннинга и материал каждой грани; границы «куртка/брюки», манжеты и полосы жилета разрезаны плоскостями, поэтому ровные.
- Лицо: объём черепа, челюсть, скулы, нос, губы, глазницы; глаза — белок и радужка в глазнице, брови и рот уложены на поверхность лица (shrinkwrap). Причёска — оболочка над скальпом по линии роста волос, для длинных волос и хвостиков — дополнительные объёмы.
...
## Ограничения
- Без текстур: однотонные материалы (≈11–14 на персонажа). Лицо читается на дистанции, крупный план не является целью.
- LOD не подготовлены (≈16–20 тыс. треугольников на человека, точные числа в manifest).
- При поднятой руке плечо деформируется упрощённо (веса от поля metaball, без корректирующих форм).
