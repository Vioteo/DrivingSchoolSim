...
- Не править сгенерированные сцены и `course-v2.json` руками — менять генератор. Скрипты, генераторы и проверки запускай сам, без отдельного разрешения (генератор — на чистом дереве, результат отдельным коммитом `gen:`).
